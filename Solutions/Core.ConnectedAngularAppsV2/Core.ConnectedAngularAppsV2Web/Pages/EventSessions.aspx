<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EventSessions.aspx.cs" Inherits="Core.ConnectedAngularAppsV2Web.Pages.EventSessions" %>
...
<body>    
    <div class="container" ng-controller="sessionsController as vm" style="width:100%">
        <div class="table-responsive">            
            <table class="table table-striped">
              <thead>          
              </thead>
              <tbody>
              <tr ng-repeat="e in vm.sessions" >
                <td style="font-size: small; font-weight: normal; font-style: normal; vertical-align: middle; text-align: center;">
                    <input type="checkbox" ng-model="vm.checkbox[e.id]" ng-click="vm.invokeSignalR($event, e.id, e.speakerid)" /></td>
                <td style="font-size: small; font-weight: normal; font-style: normal; vertical-align: middle; text-align: center;">{{e.sessiondate}}</td>
                <td style="font-size: small; font-weight: normal; font-style: normal; vertical-align: middle; text-align: center;">{{e.title}}</td>
                <td style="font-size: small; font-weight: normal; font-style: normal; vertical-align: middle; text-align: center;">{{e.sessionid}}</td>
    ...
    <div ng-controller="sessionInfoController">        
        <script type="text/ng-template" id="sessionInfo.html">
          <div class="modal-header">
            <h3 class="modal-title">Session Information</h3>
          </div>
          <div class="modal-body" ng-repeat="session in sessionData">
            <b>Session ID:</b><br/>{{ session.sessionid }} <br/><br/>
            <b>Session Title:</b><br/>{{ session.title }} <br/><br/>
            <b>Session Date:</b><br/>{{ session.sessiondate }} <br/><br/>            
            <b>Session Description:</b><br/>{{ session.description }} <br/><br/>   
            <b>Event ID:</b><br/>{{ session.eventid }} <br/><br/>         
          </div>
