<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CorporateEvents.aspx.cs" Inherits="Core.ConnSigRAngJSApps.Pages.CorporateEvents" %>

<!DOCTYPE html>

<html ng-app="app"  ng-hint>
<head>
    <title></title>
     <!--Script references. -->    
...
<body >          
    <div ng-controller="eventsController as vm" style="width:100%">
        <div ng-repeat="e in vm.events">                                     
            <dl>
                <dt class="dl-horizontal">
                    <input type="checkbox" ng-model="vm.checkbox[e.id]" ng-click="vm.updateSelection($event, e.id, e.registeredeventid)" />
                    {{e.eventdate}}  -  {{e.title}}
                </dt>                
            </dl>
            <dl>                        
                ...
                <dt class="dl-horizontal">
                    <button style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-size:x-small;"
                                ng-click="vm.invokeDeleteEvent(e.id)">
                            Delete Event
                        </button>
                </dt>                        
            </dl> 
