...
namespace OfficeDevPnP.SPOnline.Commands.Base
{
    public class SPOCmdlet : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                int healthScore = SPOnline.Core.Utils.Health.GetHealthScore(SPOnlineConnection.CurrentConnection.Url);
                ...
                else
                {
                    if (SPOnlineConnection.CurrentConnection.RetryCount != -1)
                    {
                        ...
                        while (retry <= SPOnlineConnection.CurrentConnection.RetryCount)
                        {
                            ...
                            healthScore = SPOnline.Core.Utils.Health.GetHealthScore(SPOnlineConnection.CurrentConnection.Url);
                            if (healthScore <= SPOnlineConnection.CurrentConnection.MinimalHealthScore)
                            {
                                ExecuteCmdlet();
                                break;
