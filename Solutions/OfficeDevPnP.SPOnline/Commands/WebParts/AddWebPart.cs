        ...
        protected override void ExecuteCmdlet()
        {
            if (ParameterSetName == "FILETOWIKI")
            {
                ...
                if (System.IO.File.Exists(Path))
                {
                    ...
                    {
                        SPOnline.Core.SPOWebParts.ImportWebPart(webPartString, PageUrl, ZoneId, ZoneIndex, this.SelectedWeb, ClientContext);
                    }
                    else
                    {
                        SPOnline.Core.SPOWebParts.AddWebPart(webPartString, this.SelectedWeb, PageUrl, ClientContext, Row, Column);
                    }
                }
            }
            ...
            {
                SPOnline.Core.SPOWebParts.AddWebPart(WebPart, PageUrl, ZoneId, ZoneIndex, this.SelectedWeb, ClientContext);
            }
            else if (ParameterSetName == "XMLTOWIKI")
            {
                SPOnline.Core.SPOWebParts.AddWebPart(Xml, this.SelectedWeb, PageUrl, ClientContext, Row, Column);
            }
        }
    }
}
