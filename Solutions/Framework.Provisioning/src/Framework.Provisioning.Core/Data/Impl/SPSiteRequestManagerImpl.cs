...
namespace Framework.Provisioning.Core.Data.Impl
{
    /// <summary>
    ...
    internal class SPSiteRequestManagerImpl : ISiteRequestManager, ISharePointService
    {
        ...
        public void UpdateRequestStatus(string url, SiteRequestStatus status, string statusMessage)
        {
            UsingContext(ctx =>
            {
                Log.Info("Framework.Provisioning.Core.Data.Impl.UpdateRequestStatus", "Updating Site Request Status for URL {0} to status {1}", url, status.ToString());
