...
{
    /// <summary>
    /// Extension methods for the ClientRuntimeContext class
    /// </summary>
    public static class ClientContextExtensions
    {
        /// <summary>
        /// Throttle safe executequery: will retry the ExecuteQuery command when being throttled
        /// </summary>
        /// <param name="clientContext">ClientRuntimeContext object to use</param>
        /// <param name="retryCount">How many retries will be attempted</param>
        /// <param name="delay">Delay in milliseconds. After each retry attempt this delay is multiplied by 2</param>
        public static void ExecuteQueryRetry(this ClientRuntimeContext clientContext, int retryCount = 5, int delay = 10000)
        {
            ExecuteQueryImplementation(clientContext, retryCount, delay);
        }

        private static void ExecuteQueryImplementation(ClientRuntimeContext clientContext, int retryCount = 5, int delay = 10000)
        {
            int retryAttempts = 0;
            ...
            while (retryAttempts < retryCount)
            {
                ...
                catch (WebException wex)
                {
                    ...
                    // Check is request failed do to server unavailable - http status code 503
    ...
    /// <summary>
    /// Exception that will be thrown when dealing with throttling failed
    /// </summary>
    [Serializable]
