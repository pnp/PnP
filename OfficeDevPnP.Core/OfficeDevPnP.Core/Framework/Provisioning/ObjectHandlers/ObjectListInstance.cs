...
namespace OfficeDevPnP.Core.Framework.Provisioning.ObjectHandlers
{
    public class ObjectListInstance : ObjectHandlerBase
    {
        ...
        public override void ProvisionObjects(Web web, ProvisioningTemplate template)
        {
            ...
            {
                throw new ApplicationException("No lists defined in template xml.");
            }

            var clientContext = web.Context is ClientContext ? web.Context as ClientContext : null;
            if (clientContext == null)
            {
                throw new ApplicationException("ClientContext not assigned.");
            }
            ...
            foreach (var list in template.Lists)
            {
                if (existingLists.FindIndex(x => x.Equals(UrlUtility.Combine(serverRelativeUrl, list.Url), StringComparison.OrdinalIgnoreCase)) == -1)
                {
                    ...
                    var contentTypesToRemove = new List<Microsoft.SharePoint.Client.ContentType>();
            ...
            foreach (var listInfo in createdLists.Where(listInfo => listInfo.ListInstance.FieldRefs.Any()))
            {
                foreach (var fieldRef in listInfo.ListInstance.FieldRefs)
                {
                    ...
                    if (listInfo.CreatedList.FieldExistsById(fieldRef.Id)) continue;
            ...
            foreach (var listInfo in createdLists)
            {
                ...
                foreach (var viewDoc in list.Views.Select(view => XDocument.Parse(view.SchemaXml)))
                {
                    if (viewDoc.Root == null)
                    {
                        throw new ApplicationException("Invalid XDocument.");
                    }
                ...
                if (!list.OnQuickLaunch || !list.RemoveExistingViews || list.Views.Count <= 0) continue;
        ...
        public override ProvisioningTemplate CreateEntities(Web web, ProvisioningTemplate template, ProvisioningTemplateCreationInformation creationInfo)
        {
            ...
            foreach (var item in lists)
            {
                ...
                {
                    // Check if we need to skip this list...
                    // if so let's do it before we gather all the other information for this list...improves perf
                    index = creationInfo.BaseTemplate.Lists.FindIndex(f => 
                        f.Url.Equals(item.RootFolder.ServerRelativeUrl.Substring(serverRelativeUrl.Length)) &&
                ...
                foreach (var ct in item.ContentTypes)
                {
                    ...
                    web.Context.ExecuteQueryRetry();
                    contentTypeFields.AddRange(from fieldLink in ct.FieldLinks where !fieldLink.Hidden select new FieldRef { Id = fieldLink.Id });
                    count++;
                }
