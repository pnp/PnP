...
namespace OfficeDevPnP.Core.Framework.Provisioning.ObjectHandlers
{
    public class ObjectListInstance : ObjectHandlerBase
    {
        ...
        /// <summary>
        /// Actual implementation of creating lists on existing web from given provisioning template.
        /// </summary>
        /// <param name="web">Web to add list to</param>
        /// <param name="template">ProvisioningTemplate to get the list defininions from</param>
        ...
        public override void ProvisionObjects(Web web, ProvisioningTemplate template)
        {
            ...
            {
                var index = -1;
                index = existingLists.FindIndex( x =>
                            x.Equals(UrlUtility.Combine(serverRelativeUrl, list.Url), StringComparison.OrdinalIgnoreCase));
                ...
                if (index == -1)
                {
                    var lci = new ListCreationInformation
                    {
                        ...
                        Url = list.Url.ToParsedString(),
                        TemplateFeatureId = list.TemplateFeatureID
                    };

                    var createdList = web.Lists.Add(lci);
                    createdList.Update();
                ...
                {
                    Log.Warning(Constants.LOGGING_SOURCE_FRAMEWORK_PROVISIONING, "ProvisionDeltaList NotImplemented");
                }
            ...
            foreach (var listInfo in createdLists)
            {
                if (listInfo.ListInstance.Fields.Any())
                {
                    foreach (var field in listInfo.ListInstance.Fields)
                    {
                        ...
                        if (listInfo.CreatedList.FieldExistsById(fieldGuid)) continue;
        ...
        /// <summary>
        /// Actual implementation of adding list definitions to given provisioning template.
        /// </summary>
        /// <param name="web">Web to read the list definitions from</param>
        /// <param name="template">ProvisioningTemplate to add the list defininions to</param>
        /// <param name="creationInfo">ProvisioningTemplateCreationInformation to check against if list is defined in base template</param>
        /// <returns>ProvisioningTemplate with added lists definitions</returns>
        public override ProvisioningTemplate CreateEntities(Web web, ProvisioningTemplate template, ProvisioningTemplateCreationInformation creationInfo)
        {
            ...
            foreach (var list in lists)
            {
                ...
                {
                    // Check if we need to do a delta processing for this list...if so let's do it before we gather all the other information for this list...improves perf
                    index = creationInfo.BaseTemplate.Lists.FindIndex(f => f.Url.Equals(list.RootFolder.ServerRelativeUrl.Substring(serverRelativeUrl.Length)) &&
                                                                           f.TemplateType.Equals(list.BaseTemplate));
                }

                if (index == -1)
                {
                    AddNewListToTemplate(web, template, list, serverRelativeUrl);
                }
                else
                {
                    AddDeltaListToTemplate(web, template, list, serverRelativeUrl, creationInfo, index);
                }
            }
        ...
        private void AddNewListToTemplate(Web web, ProvisioningTemplate template, List list, string serverRelativeUrl)
        {
            var contentTypeFields = new List<FieldRef>();
            var newListInstance = new ListInstance
            {
                Description = list.Description,
                ...
                DocumentTemplate = Tokenize(list.DocumentTemplateUrl, web.Url),
                ContentTypesEnabled = list.ContentTypesEnabled,
                Url = list.RootFolder.ServerRelativeUrl.Substring(serverRelativeUrl.Length).TrimStart('/'),
                TemplateFeatureID = list.TemplateFeatureId,
                EnableAttachments = list.EnableAttachments,
                OnQuickLaunch = list.OnQuickLaunch,
                MaxVersionLimit =
                    list.IsObjectPropertyInstantiated("MajorVersionLimit") ? list.MajorVersionLimit : 0,
                EnableMinorVersions = list.EnableMinorVersions,
                MinorVersionLimit =
                    list.IsObjectPropertyInstantiated("MajorWithMinorVersionsLimit")
            ...
            foreach (var ct in list.ContentTypes)
            {
                ...
                if (ct.Parent != null)
                {
                    ...
                    {
                        newListInstance.ContentTypeBindings.Add(new ContentTypeBinding()
                        {
                            ContentTypeId = ct.Parent.StringId,
                            Default = count == 0
                        });
                    }
                }
                ...
                web.Context.ExecuteQueryRetry();
                contentTypeFields.AddRange(from fieldLink in ct.FieldLinks
                    where !fieldLink.Hidden
                    select new FieldRef {Id = fieldLink.Id});
                count++;
            }

            foreach (var view in list.Views.Where(view => !view.Hidden))
            {
                newListInstance.Views.Add(new View() {SchemaXml = view.ListViewXml});
            }
            ...
            foreach (var field in list.Fields.Where(field => !field.Hidden))
            {
                if (siteColumns.FirstOrDefault(sc => sc.Id == field.Id) != null)
                {
                    var addField = true;
                    if (list.ContentTypesEnabled && contentTypeFields.FirstOrDefault(c => c.Id == field.Id) == null)
                    {
                    ...
                    if (addField)
                    {
                        newListInstance.FieldRefs.Add(new FieldRef(field.InternalName)
                        {
                            Id = field.Id,
                ...
                {
                    newListInstance.Fields.Add((new Model.Field {SchemaXml = field.SchemaXml}));
                }
            }
            template.Lists.Add(newListInstance);
        }

        private void AddDeltaListToTemplate(Web web, ProvisioningTemplate template, List list, string serverRelativeUrl, ProvisioningTemplateCreationInformation creationInfo, int basetemplateIndex)
        {
            //throw new NotImplementedException();
            Log.Warning(Constants.LOGGING_SOURCE_FRAMEWORK_PROVISIONING, "GetDeltaListTemplate NotImplemented");
        }
