...
namespace OfficeDevPnP.Core.Framework.Provisioning.ObjectHandlers
{
    internal class ObjectListInstance : ObjectHandlerBase
    {
        ...
        public override void ProvisionObjects(Web web, ProvisioningTemplate template)
        {
            ...
            var updatedLists = new List<ListInfo>();
            ...
            foreach (var list in template.Lists)
            {
                if (existingLists.FindIndex(x => x.Equals(UrlUtility.Combine(serverRelativeUrl, list.Url), StringComparison.OrdinalIgnoreCase)) == -1)
                {
                    ...
                    if (list.EnableVersioning)
                    {
                        ...
                        if (createdList.BaseTemplate == (int)ListTemplateType.DocumentLibrary)
                        {
                            ...
                            if (list.EnableMinorVersions)
                            {
                                ...
                                // Set only if enabled, otherwise you'll get exception due setting value to zero.
                    ...
                    TokenParser.AddToken(new ListUrlToken(web, list.Title,
                        createdList.RootFolder.ServerRelativeUrl.Substring(web.ServerRelativeUrl.Length + 1)));
                }
                else
                {
                    // not implemented list should updated but check the list before for diffs
                }
        ...
        public override ProvisioningTemplate CreateEntities(Web web, ProvisioningTemplate template, ProvisioningTemplateCreationInformation creationInfo)
        {
            ...
            foreach (var item in lists)
            {
                ...
                {
                    if (!list.Equals(creationInfo.BaseTemplate.Lists[index])) // existing list changed
                    {
                        template.Lists.Add(list);
                    }
                }
        ...
        {
            if (_willExtract.HasValue) return _willExtract.Value;
