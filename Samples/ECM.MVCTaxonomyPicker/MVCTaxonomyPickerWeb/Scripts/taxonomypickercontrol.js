...
(function (CAMControl) {
    ...
    function TermSet(options) {
        ...
        this.LCID = (options.lcid) ? options.lcid : 1033;
    ...
    jQuery.extend(TermSet.prototype, {
        ...
        termsLoadedSuccess: function () {
            ...
            this.Name = this.RawTermSet.Name;
            ...
            if (this.RawTermSet.CustomSortOrder) {
                sortOrder = this.RawTermSet.CustomSortOrder;
            }
        ...
        termsLoadedFailed: function (event, args) {
            ...
            alert(TaxonomyPickerConsts.TERMSET_LOAD_FAILED + ": " + args.get_message());
        },
        ...
        addTerm: function (label, taxpicker, parentTermId) {
            ...
            if (this.termExists((parentTermId == undefined) ? label : parent.Name + ';' + label)) {
                ...
                if (parentTermId == undefined) {
                    //add Term to termset
                    newData.TermSetId = this.RawTermSet.Id
                }
                else {
                    newData.ParentTermId = parentTermId
                }
                ...
                $.ajax({
                    ...
                    error: function (textStatus, errorThrown) {
                        taxpicker.termAddFailed(textStatus)
                    }
                });
            }
        },
    ...
    function TaxonomyPicker(control, options, changeCallback) {
        ...
        this._suggestionContainer = jQuery('#' + this._controlId.replace('Control', 'Suggestions'));  //the suggestions container for the taxonomy picker
        this._hiddenValidated = jQuery(control).find('input'); //the hidden control that contains all validated term selections
    ...
    jQuery.extend(TaxonomyPicker.prototype, {
        ...
        termAddSuccess: function (event, args) {
            ...
            var newTerm = this.TermSet.NewTerm;
            ...
            parentTerm.push(newTerm);//add to the hierarchy terms list
        ...
        checkExternalClick: function (event) {
            ...
            if (!jQuery.contains(this._control, event.target) && !jQuery.contains(this._suggestionContainer[0], event.target) && this._dialog != null && !jQuery.contains(this._dialog[0], event.target)) {
                var rawText = this._editor.text(); //the raw text in the editor (html stripped out)
    ...
    jQuery.fn.taxpicker = function (options, changeCallback) {
