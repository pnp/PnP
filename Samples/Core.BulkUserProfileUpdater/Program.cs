namespace Contoso.Core.BulkUserProfileUpdater
{
    using System;
    ...
    public class Program
    {
        ...
        /// </summary>
        private static Type basePropertyType = typeof(Contoso.Core.PropertyBase);
        ...
        {
            return Assembly.GetExecutingAssembly().GetExportedTypes().Where(p => p.IsSubclassOf(basePropertyType) || p.IsSubclassOf(baseActionType) || p.IsSubclassOf(baseLoggerType)).ToArray();
        }
        ...
        private static void Main(string[] args)
        {
            ...
            finally
            {
                ...
                if (logHelper != null)
                {
                    logHelper.LogVerbose(string.Format("Operation completed. Execution time (s) {0}", sw.Elapsed.TotalSeconds));
                    logHelper.Dispose();
                }
            }
        }
        ...
        private static void RecurseActions( ActionCollection collection, LogHelper logHelper, DateTime CurrentTime, BaseAction parentAction = null)
        {
            ...
            {
                logHelper.LogVerbose(string.Format("Started '{0}'", action.GetType().FullName));
                ...
                logHelper.LogVerbose(string.Format("Completed '{0}'", action.GetType().FullName));
            }
        }
